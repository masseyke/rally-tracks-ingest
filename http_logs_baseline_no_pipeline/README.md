## HTTP logs baseline no pipeline

Track that indexes HTTP logs without any ingest node pipeline. 
...
Further, this track is only focused on optimized, yet (ideally) realistic settings for high volume ingestion. No ingest node pipelines are called for this track, and is intended to be used a baseline for other "http_logs_*" tracks.  Query performance is not tested as part of this track. 
